...
GO_annotations <- read.table("GOs_all_methylated_genes_as_background.txt")
...
my_genes <- read.csv("./gene_lists/diff_meth_weighted_genes.txt",
                     header=T)
...
write.table(REVIGO,"enriched_GOs_diffmeth.txt",row.names = F,sep = "\t",quote = F)
