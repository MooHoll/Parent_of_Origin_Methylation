...
sample.list <- list("875_1_drone.CpG_report.merged_CpG_evidence.cov",
                    ...
                    "882_0_drone.CpG_report.merged_CpG_evidence.cov",
...
meth_all_data <- unite(filtered_data, destrand=F, min.per.group = 12L) 
nrow(meth_all_data) 
# all data: 159 (with 12/16 samples = 5203) 
...
df_meth_all[,c(5,8,11,14,17,20,23,26,29,32,35,38,41,44,47,50,53,56,59,62,65,68,71,74,77,80,83,86,89,92,95,98)][is.na
              (df_meth_all[,c(5,8,11,14,17,20,23,26,29,32,35,38,41,44,47,50,53,56,59,62,65,68,71,74,77,80,83,86,89,92,95,98)])] <- 100
...
df_meth_all[,c(6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63,66,69,72,75,78,81,84,87,90,93,96,99)][is.na
              (df_meth_all[,c(6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57,60,63,66,69,72,75,78,81,84,87,90,93,96,99)])] <- 1
...
aa <- df_meth_all[,77:78]
ba <- df_meth_all[,80:81]
ca <- df_meth_all[,83:84]
da <- df_meth_all[,86:87]
ea <- df_meth_all[,89:90]
fa <- df_meth_all[,92:93]
ga <- df_meth_all[,95:96]
ha <- df_meth_all[,98:99]
...
# all data: 0 (with 12/16 samples = 18) 
# cross 1: 26 (with 48 samples = ) 
# cross 2: 22 (with 4/8 samples = ) 
...
covariates <- data.frame(colony=c("875","875","875","875","875","875","875","875",
...
covariates <- data.frame(colony=c("882","882","882","882","882","882","882","882",
                                  "894","894","894","894","894","894","894","894"))
...
diff_meth_5 <- getMethylDiff(diff_meth, difference=5, pvalue=0.05)
